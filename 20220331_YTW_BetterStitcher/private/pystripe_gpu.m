function [img_filtered] = pystripe_gpu(input_img,varargin)
...
for ii = 1:max_level
    ...
    bch_filtered = irfft_gpu(bch_fft .* bg, size(bch,2),2);
    fch_filtered = irfft_gpu(fch_fft .* fg, size(fch,2),2);
...
background = waverec2_gpu(background_c, background_s, wavelet);
foreground = waverec2_gpu(foreground_c, foreground_s, wavelet);
...
img_filtered = gather(img_filtered);
