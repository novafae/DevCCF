function y = upsconv2_gpu(x,f,s,dwtARG1,dwtARG2)
...
if ndimX<3
    ...
    function y = upsconv2ONE(z)
        ...
        if ~perFLAG
            y = dyadup_gpu(z,'row',0);
            y = conv2(y',f{1}(:)','full'); y = y';
            y = dyadup_gpu(y,'col',0);
            y = conv2(y ,f{2}(:)','full');
            y = wkeep2_gpu(y,s,'c',dwtSHIFT);
        else
            y = dyadup_gpu(z,'row',0,1);
            y = wextend('addrow','per',y,lf/2);
            y = conv2(y',f{1}(:)','full'); y = y';
            y = y(lf:lf+s(1)-1,:);
            %-------------------------------------------
            y = dyadup_gpu(y,'col',0,1);
