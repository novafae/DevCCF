...
xxx_min = floor(min(center_points.xxx  + min(center_points.xxx_shift,[],2)) - tiling_info.x_pixel./2)  ;
xxx_max = ceil(max(center_points.xxx  + max(center_points.xxx_shift,[],2)) + tiling_info.x_pixel./2);
yyy_min = floor(min(center_points.yyy  + min(center_points.yyy_shift,[],2)) - tiling_info.y_pixel./2);
yyy_max = ceil(max(center_points.yyy  + max(center_points.yyy_shift,[],2)) + tiling_info.y_pixel./2);
...
% mmx = 1:100:size(new_reading_file_ind,3).*size(new_reading_file_ind,4);
% parfor mmmm = 1:length(mmx)
%     mm = mmx(mmmm);

% skipping_img = z_truncate.*5;

%for mm_p = 1:12
%    mm = round(skipping_img./2) + skipping_img.*mm_p;
%parfor mm = 1:1:size(new_reading_file_ind,3).*size(new_reading_file_ind,4)
for mm = 500:500:size(new_reading_file_ind,3).*size(new_reading_file_ind,4)

    [kk_prime,ll] = ind2sub([size(new_reading_file_ind,3), size(new_reading_file_ind,4)],mm);
    img_temp = zeros([final_img_size,3]);
...
%                 img_temp_2 = imtranslate(img_temp_2,[small_shift_xxx, small_shift_yyy],'OutputView','full');
                ...
                img_temp(ping_location_xxx+1:ping_location_xxx+size(img_temp_2,1),ping_location_yyy+1:ping_location_yyy+size(img_temp_2,2),mod((ii+jj),2)+1) = ...
                    img_temp(ping_location_xxx+1:ping_location_xxx+size(img_temp_2,1),ping_location_yyy+1:ping_location_yyy+size(img_temp_2,2),mod((ii+jj),2)+1) + img_temp_2;
            end
        end
    end
    %{
